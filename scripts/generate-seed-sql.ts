...
/**
 * Rows are emitted in batches rather than one enormous multi-row INSERT.
 *
 * A single statement carrying all 19 venues runs to ~70 KB, which is awkward to
 * localize when the server rejects one row, and large enough to defeat static
 * parsers. Batching keeps each statement small enough to verify and to blame.
 */
...
for (const batch of chunk(venues, 4)) {
  ...
  lines.push(batch.map((v) => venueRow(v, false)).join(',\n'));
  lines.push(VENUE_UPSERT);
  lines.push('');
}
...
const REVIEW_UPSERT = `on conflict (seed_key) do update set
  venue_id = excluded.venue_id,
...
for (const batch of chunk(reviews, 10)) {
  lines.push(reviewCols);
  lines.push(batch.map(reviewRow).join(',\n'));
  lines.push(REVIEW_UPSERT);
  lines.push('');
}
